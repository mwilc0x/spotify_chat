...
app.controller('MainCtrl', function MainCtrl($log, $scope, $modal, $compile, messageType, chatService, audio) {
    ...
    function onSongRequested(songRequest) {
        ...
        $scope.messages.push({ name: songRequest.user, text: songRequest.message });
        $scope.messages.push({ name: "Bot", text: "Now playing " + songRequest.title });
    ...
    $scope.submit = function () {
        if (this.text) {
            ...
            var message = JSON.stringify({ data: msgType.data, text: msgType.text, user: $scope.user.name, message: msgType.message });
            chatService.send(message);
